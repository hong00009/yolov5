...
def food_info(post):
    ...
    if post.detection_result is None and not user_food_nutritions_list.exists():
        # 검출된 객체가 없고, 연관된 UserFoodNutritions도 없으면
        print('**food_info함수 : 객체검출X')
        return [], [], 0, 0, 0

    if user_food_nutritions_list:
        # 영양정보 추출하여 리스트로 저장
        ...
        # 차트에 그릴 정보 json형식으로 반환받음
         ...
         print('**food_info함수 : 객체검출O 음식정보X(사용자삭제)')
         food_name_list = []
         nutrition_info_list = []
         total_chart_info_json, each_chart_info_json , percentage= chart(nutrition_info_list)
